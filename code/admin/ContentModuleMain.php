...
class ContentModuleMain extends LeftAndMain implements PermissionProvider {
	...
	public function LinkModuleEdit($id = null) {
		...
		return $this->LinkWithSearch(
			Controller::join_links(singleton('ContentModuleEditController')->Link('show'), $id)
		);
	}
	...
	public function ModuleTypesList() {
		...
		foreach($modules as $instance) {
			...
			if($instance->stat('need_permission') && !$this->can($instance->stat('need_permission'))) continue;
	...
	public function getEditForm($id = null, $fields = null) {
		...
		if($record) {
			...
			if($actionsFlattened) foreach($actionsFlattened as $action) {
				$action->setUseButtonTag(true);
			}
			...
			return new Form($this, "EditForm", new FieldList(
				new LabelField('ModuleDoesntExistLabel',_t('ContentModule.MODULENOTEXISTS',"This module doesn't exist"))), new FieldList()
			);
		}
        ...
        public function getModulesGridField($params, $module) {
                ...
                $columns->setFieldFormatting(array(
                        'Title' => function($value, &$item) use($controller) {
                                return '<a class="action-detail" href="' . singleton('ContentModuleEditController')->Link('show') . '/' . $item->ID . '">' . $item->Title . '</a>';
                        }
                ));
	...
	public function revert($data, $form) {
		...
		if(!$restoredPage) 	return new SS_HTTPResponse("ContentModule #$id not found", 400);
