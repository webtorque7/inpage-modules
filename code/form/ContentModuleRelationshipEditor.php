...
class ContentModuleRelationshipEditor extends FormField
{
    ...
    public function __construct($name, $title = null, $relationship = null, $record = null, $fieldList = null)
    {
        ...
        if ($fieldList) {
            ...
            if ($className = $this->getItemClassName()) {
    ...
    {
        return ($this->record && $this->record->hasMethod(
                        $this->_relationship
                )) ? $this->record->{$this->_relationship}() : null;
    }
    ...
    public function getItems()
    {
        if ($this->record && $this->record->hasMethod($this->_relationship)) {
            ...
            if ($items && $items->count()) {
                foreach ($items as $item) {
                    $return->push(
                            ContentModuleRelationshipEditor_Item::create($this, $item)->setCanEdit($this->getCanEdit())
                    );
                }
            }
        ...
        user_error(
                sprintf('Relationship %s doesn\'t exist on class %s', $this->_relationship, $this->record->ClassName)
        );
    }
                    ...
                    if ($record->hasMethod('canEdit') && $record->canEdit()) {
                        $actions->push(
                                FormAction::create('save', _t('CMSMain.SAVE', 'Save'))
                                        ->addExtraClass('ss-ui-action-constructive')->setAttribute(
                                                'data-icon',
                                                'accept'
                                        )
                        );
